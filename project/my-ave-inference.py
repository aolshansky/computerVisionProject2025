...
def enforce_rank2(F):
    """
    Ensures the fundamental matrix F has rank 2 by setting the smallest singular value to zero.

    Parameters:
        F (numpy.ndarray): 3x3 fundamental matrix.

    Returns:
        numpy.ndarray: Rank-2 fundamental matrix.
    """
...
def compute_ave_F(wei_list, F_list):
    # -------- Average F:
    if len(wei_list) == 0:
        return np.eye(3, 3)
    ...
    for w in wei_list:
        wei_sum = +w
    for j, w in enumerate(wei_list):
        F_ave = w / wei_sum * F_list[j]
...
for i, row in enumerate(test_samples):
    ...
    try:
        ...
        if F is not None:
            wei = 1.0/compute_reprojection_error(F, kpts1, kpts2, inlier_mask)
            F_list.append(F)
            wei_list.append(wei)
    except:
        F = np.eys(3, 3)
    ...
    idx_x = {0: [0, half_w_a], 1: [half_w_a - pad_w, W_A-1]}
    idx_y = {0: [0, half_h_a], 1: [half_h_a - pad_h, H_A-1]}
        ...
        for j in [0, 1]:
            ...
            try:
                F_t, inlier_mask = cv2.findFundamentalMat(
                    kpts1_.cpu().numpy(), kpts2_.cpu().numpy(),
                    method=cv2.USAC_MAGSAC, ransacReprojThreshold=0.25,
                    confidence=0.99999, maxIters=100000)
                if F_t is not None:
                    wei = 1.0/(compute_reprojection_error(F_t, kpts1_, kpts2_, inlier_mask) + 1e-6)
                    F_list.append(F_t)
                    wei_list.append(wei)
            except:
    ...
    # -------- Average F:
    F_ave = compute_ave_F(wei_list, F_list)

    # ----- F constraint (rank2)
    F_ave = enforce_rank2(F_ave)

    F_dict[sample_id] = F_ave
